...
This script integrates the core functions of Mask_RCNN (https://github.com/matterport/Mask_RCNN) into a single file, with 
...
def data_generator(dataset, config, shuffle=True, augmentation=None, random_rois=0,
    ...
    no_augmentation_sources : list of sources to exclude for augmentation
    """
    b = 0
    image_index = -1
    ...
    while True:
        try:
            ...
            if gt_boxes.shape[0] > config.MAX_GT_INSTANCES:
                ids = np.random.choice(np.arange(gt_boxes.shape[0]), config.MAX_GT_INSTANCES, 
                                       replace=False)
